...
#### Check F: Probe — verify law detects violations

Run the probe to verify the law actually catches what it claims:

```bash
bash commands/chp-law probe <law-name>
```

- If probe reports `FAIL:missed-violation` — the check doesn't detect its own pattern. This is a broken law.
- If probe reports `FAIL:false-positive` — the check flags clean content. The pattern is too broad.
- If the law has a `probes.json`, explicit test cases are also run.

**Fix:** For missed violations, tighten the regex or add the missing pattern. For false positives, narrow the scope or add exclude patterns.
