...
- User says "new law", "add a rule", "enforce", "create a law", "write a law"
- User wants to propose rules, brainstorm enforcement ideas, or tune existing laws
- User says "come up with laws" or "what should we enforce"
- After `chp:decompose-laws` finishes producing atomic checks that need implementation
- Refining an existing law — adding patterns, changing severity, adjusting hooks
...
After testing, run the probe to verify the law catches real violations automatically:

```bash
bash commands/chp-law probe <law-name>
```

The probe plants known violations and clean content in a temp repo, runs verify.sh, and reports whether the law actually detects what it claims.

If the law has edge cases or known patterns to test, create a `probes.json` alongside the other law files:

```json
{
  "probes": [
    {
      "description": "Known violation case",
      "content": "const apiKey = \"sk_live_example_key_value\";",
      "extension": "js",
      "shouldDetect": true
    },
    {
      "description": "Clean code (no violation)",
...
**Fix any probe failures before proceeding.** A law that can't catch its own violations is worse than no law.
